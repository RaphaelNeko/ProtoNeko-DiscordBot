	...
	run: async ({ interaction }) => {
		try {
			...
			if (!guildConfiguration?.suggestionChannelIds.length) {
				await interaction.reply('This server has not been configured to use suggestions.');
				return;
			}

			if (!guildConfiguration.suggestionChannelIds.includes(interaction.channel.id)) {
				await interaction.reply(`You cannot suggest anything here! Try there instead: ${guildConfiguration.suggestionChannelIds.map((id) => `<#${id}>`).join(', ')}`);
				return;
			}

			const modal = new ModalBuilder().setTitle('Create a suggestion').setCustomId(`suggestion-${interaction.user.id}`);
			const textInput = new TextInputBuilder().setCustomId('suggestion-input').setLabel('Suggestion').setStyle(TextInputStyle.Paragraph).setRequired(true).setMaxLength(1000);
			...
			modalInteraction.editReply('☑️ Your suggestion has been created! ✨');
			...
			console.log(`⚠️ Error in /suggest: ${error}`);
		}
	}
}