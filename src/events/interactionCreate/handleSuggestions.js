	...
	try {
		...
		if (action == 'approve') {
			if (!interaction.memberPermissions.has('Administrator')) {
				await interaction.editReply("You don't have the permissions to approve suggestions.");
				return;
			}
		...
		if (action == 'reject') {
			if (!interaction.memberPermissions.has('Administrator')) {
				await interaction.editReply("You don't have the permissions to reject suggestions.");
				return;
			}
		...
		if (action == 'upvote') {
			...
			if (hasVoted) {
				await interaction.editReply('⚠️ You already voted!');
				return;
			}
			...
			interaction.editReply('👍 You upvoted! 👍');
		...
		if (action == 'downvote') {
			...
			interaction.editReply('👎 You downvoted 👎');
		...
		console.log(`⚠️ Error in handleSuggestion.js: ${error}`);
	}
}